...
class JUri
{
	 ...
	 * @since   11.1
	 */
	public static function base($pathonly = false,$path=null) //[hw2] passing custom path
	{
		...
		if (empty(self::$base))
		{
			...
			if (trim($live_site) != '')
			{
				...
				if (my_defined('JPATH_BASE') && my_defined('JPATH_ADMINISTRATOR'))
				{
			...
			else
			{
				...
				self::$base['path'] = is_null($path) ? rtrim(dirname($script_name), '/\\') : $path; // [hw2]
			}
		}
