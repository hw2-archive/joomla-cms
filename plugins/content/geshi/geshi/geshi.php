...
my_define_d('GESHI_VERSION', '1.0.8.10');

// Define the root directory for the GeSHi code tree
if (!my_defined('GESHI_ROOT')) {
    /** The root directory for GeSHi */
    my_define_d('GESHI_ROOT', dirname(__FILE__) . DIRECTORY_SEPARATOR);
}
/** The language file directory for GeSHi
    @access private */
my_define_d('GESHI_LANG_ROOT', GESHI_ROOT . 'geshi' . DIRECTORY_SEPARATOR);

// Define if GeSHi should be paranoid about security
if (!my_defined('GESHI_SECURITY_PARANOID')) {
    /** Tells GeSHi to be paranoid about security settings */
    my_define_d('GESHI_SECURITY_PARANOID', false);
}

// Line numbers - use with enable_line_numbers()
/** Use no line numbers when building the result */
my_define_d('GESHI_NO_LINE_NUMBERS', 0);
/** Use normal line numbers when building the result */
my_define_d('GESHI_NORMAL_LINE_NUMBERS', 1);
/** Use fancy line numbers when building the result */
my_define_d('GESHI_FANCY_LINE_NUMBERS', 2);

// Container HTML type
/** Use nothing to surround the source */
my_define_d('GESHI_HEADER_NONE', 0);
/** Use a "div" to surround the source */
my_define_d('GESHI_HEADER_DIV', 1);
/** Use a "pre" to surround the source */
my_define_d('GESHI_HEADER_PRE', 2);
/** Use a pre to wrap lines when line numbers are enabled or to wrap the whole code. */
my_define_d('GESHI_HEADER_PRE_VALID', 3);
/**
 ...
 */
my_define_d('GESHI_HEADER_PRE_TABLE', 4);

// Capatalisation constants
/** Lowercase keywords found */
my_define_d('GESHI_CAPS_NO_CHANGE', 0);
/** Uppercase keywords found */
my_define_d('GESHI_CAPS_UPPER', 1);
/** Leave keywords found as the case that they are */
my_define_d('GESHI_CAPS_LOWER', 2);

// Link style constants
/** Links in the source in the :link state */
my_define_d('GESHI_LINK', 0);
/** Links in the source in the :hover state */
my_define_d('GESHI_HOVER', 1);
/** Links in the source in the :active state */
my_define_d('GESHI_ACTIVE', 2);
/** Links in the source in the :visited state */
my_define_d('GESHI_VISITED', 3);
...
my_define_d('GESHI_START_IMPORTANT', '<BEGIN GeSHi>');
/** The ender for important parts of the source */
my_define_d('GESHI_END_IMPORTANT', '<END GeSHi>');
...
/** some old PHP / PCRE subpatterns only support up to xxx subpatterns in
    ...
    **/
my_define_d('GESHI_MAX_PCRE_SUBPATTERNS', 500);
/** it's also important not to generate too long regular expressions
    ...
    @see GeSHi->optimize_regexp_list()
    **/
my_define_d('GESHI_MAX_PCRE_LENGTH', 12288);

//Number format specification
/** Basic number format for integers */
my_define_d('GESHI_NUMBER_INT_BASIC', 1);        //Default integers \d+
/** Enhanced number format for integers like seen in C */
my_define_d('GESHI_NUMBER_INT_CSTYLE', 2);       //Default C-Style \d+[lL]?
/** Number format to highlight binary numbers with a suffix "b" */
my_define_d('GESHI_NUMBER_BIN_SUFFIX', 16);           //[01]+[bB]
/** Number format to highlight binary numbers with a prefix % */
my_define_d('GESHI_NUMBER_BIN_PREFIX_PERCENT', 32);   //%[01]+
/** Number format to highlight binary numbers with a prefix 0b (C) */
my_define_d('GESHI_NUMBER_BIN_PREFIX_0B', 64);        //0b[01]+
/** Number format to highlight octal numbers with a leading zero */
my_define_d('GESHI_NUMBER_OCT_PREFIX', 256);           //0[0-7]+
/** Number format to highlight octal numbers with a prefix 0o (logtalk) */
my_define_d('GESHI_NUMBER_OCT_PREFIX_0O', 512);           //0[0-7]+
/** Number format to highlight octal numbers with a leading @ (Used in HiSofts Devpac series). */
my_define_d('GESHI_NUMBER_OCT_PREFIX_AT', 1024);           //@[0-7]+
/** Number format to highlight octal numbers with a suffix of o */
my_define_d('GESHI_NUMBER_OCT_SUFFIX', 2048);           //[0-7]+[oO]
/** Number format to highlight hex numbers with a prefix 0x */
my_define_d('GESHI_NUMBER_HEX_PREFIX', 4096);           //0x[0-9a-fA-F]+
/** Number format to highlight hex numbers with a prefix $ */
my_define_d('GESHI_NUMBER_HEX_PREFIX_DOLLAR', 8192);           //$[0-9a-fA-F]+
/** Number format to highlight hex numbers with a suffix of h */
my_define_d('GESHI_NUMBER_HEX_SUFFIX', 16384);           //[0-9][0-9a-fA-F]*h
/** Number format to highlight floating-point numbers without support for scientific notation */
my_define_d('GESHI_NUMBER_FLT_NONSCI', 65536);          //\d+\.\d+
/** Number format to highlight floating-point numbers without support for scientific notation */
my_define_d('GESHI_NUMBER_FLT_NONSCI_F', 131072);       //\d+(\.\d+)?f
/** Number format to highlight floating-point numbers with support for scientific notation (E) and optional leading zero */
my_define_d('GESHI_NUMBER_FLT_SCI_SHORT', 262144);      //\.\d+e\d+
/** Number format to highlight floating-point numbers with support for scientific notation (E) and required leading digit */
my_define_d('GESHI_NUMBER_FLT_SCI_ZERO', 524288);       //\d+(\.\d+)?e\d+
 ...
 * @deprecated
 */
my_define_d('GESHI_ERROR_NO_INPUT', 1);
/** The language specified does not exist */
my_define_d('GESHI_ERROR_NO_SUCH_LANG', 2);
/** GeSHi could not open a file for reading (generally a language file) */
my_define_d('GESHI_ERROR_FILE_NOT_READABLE', 3);
/** The header type passed to {@link GeSHi->set_header_type()} was invalid */
my_define_d('GESHI_ERROR_INVALID_HEADER_TYPE', 4);
/** The line number type passed to {@link GeSHi->enable_line_numbers()} was invalid */
my_define_d('GESHI_ERROR_INVALID_LINE_NUMBER_TYPE', 5);
/**#@-*/
...
//        $result = substr($result, 1, -1);
    ...
    function parse_non_string_part($stuff_to_parse) {
        ...
        foreach (array_keys($this->language_data['REGEXPS']) as $key) {
            if ($this->lexic_permissions['REGEXPS'][$key]) {
                if (my_is_callable_d($this->language_data['STYLES']['REGEXPS'][$key])) {
                    $this->_rx_key = $key;
        ...
        for ($i = 1; $i < $narrays; $i ++) {
            foreach ($arrays[$i] as $key => $value) {
                if (is_array($value) && isset($ret[$key])) {
                    // if $ret[$key] is not an array you try to merge an scalar value with an array - the result is not my_defined(incompatible arrays)
